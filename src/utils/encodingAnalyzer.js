...
export class EncodingAnalyzer {
  ...
  static analyzeStrength(originalText, encodedText, encoder) {
    ...
    if (encoder.reversible) {
      score += 20;
      factors.push({ name: 'Reversible', impact: 20, description: 'Can be decoded' });
    } else {
      score += 50;
      factors.push({ name: 'Non-reversible', impact: 50, description: 'Cannot be decoded' });
    }
    ...
    if (lengthRatio > 2) {
      score += 20;
      factors.push({ name: 'Significant expansion', impact: 20, description: `${lengthRatio.toFixed(1)}x longer` });
    } else if (lengthRatio > 1.5) {
      score += 10;
      factors.push({ name: 'Moderate expansion', impact: 10, description: `${lengthRatio.toFixed(1)}x longer` });
    } else {
      score += 5;
      factors.push({ name: 'Minimal expansion', impact: 5, description: `${lengthRatio.toFixed(1)}x longer` });
    }
    ...
    if (diversityRatio > 0.5) {
      score += 15;
      factors.push({ name: 'High diversity', impact: 15, description: `${uniqueChars} unique characters` });
    } else if (diversityRatio > 0.3) {
      score += 10;
      factors.push({ name: 'Medium diversity', impact: 10, description: `${uniqueChars} unique characters` });
    } else {
      score += 5;
      factors.push({ name: 'Low diversity', impact: 5, description: `${uniqueChars} unique characters` });
    }
