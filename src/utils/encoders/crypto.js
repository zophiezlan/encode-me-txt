...
export const encodeArgon2Style = (text) => {
  return text
    ...
    .map((char, idx) => {
      ...
      const salt = btoa(String.fromCharCode(code, idx, code + idx)).slice(
        0,
        22,
      );
      const hash = btoa(
        String.fromCharCode(
          ...Array.from({ length: 32 }, (_, i) => (code * (i + 1) * 997) % 256),
        ),
      ).slice(0, 43);
...
export const encodeBCryptStyle = (text) => {
  return text
    ...
    .map((char, idx) => {
      ...
      const hash = btoa(
        String.fromCharCode(
          ...Array.from({ length: 31 }, (_, i) => (code * (i + 1) * 991) % 256),
        ),
      ).slice(0, 31);
...
export const encodeSSHKeyStyle = (text) => {
  return text
    ...
    .map((char, idx) => {
      const code = char.charCodeAt(0);
      const keyType = ["ssh-rsa", "ssh-ed25519", "ecdsa-sha2-nistp256"][
        code % 3
      ];
      const keyData = btoa(
        String.fromCharCode(
  ...
  const bases = [
    {
      prefix: "f",
      name: "base16",
      encode: (s) => Buffer.from(s).toString("hex"),
    },
