      ...
      for (let i = 0; i < signers; i++) {
        const sig = btoa(
          String.fromCharCode(
            ...Array.from(
              { length: 64 },
              (_, j) => (code * (j + 1) * (1117 + i * 10) + idx) % 256,
            ),
          ),
        ).slice(0, 88);
...
export const encodeXMLDSig = (text) => {
  return text
    ...
    .map((char, idx) => {
      const code = char.charCodeAt(0);
      const digestValue = btoa(
        String.fromCharCode(
          ...Array.from(
            { length: 32 },
            (_, i) => (code * (i + 1) * 1123) % 256,
          ),
        ),
      );
...
export const encodeSAMLAssertion = (text) => {
  return text
    ...
    .map((char, idx) => {
      ...
      const assertionId =
        `_${btoa(String.fromCharCode(code, idx)).replace(/[^a-zA-Z0-9]/g, "")}`.slice(
          0,
          40,
        );
  ...
  return text
    ...
    .map((char, idx) => {
      ...
      const hmac = Array.from(
        { length: algo.includes("512") ? 64 : algo.includes("256") ? 32 : 20 },
        (_, i) =>
...
export const encodeAWSSignature = (text) => {
  return text
    ...
    .map((char, idx) => {
      ...
      const credential = `AKIA${btoa(String.fromCharCode(code))
        .replace(/[^A-Z0-9]/g, "")
        .slice(0, 16)}/${dateStr}/${region}/${service}/aws4_request`;
...
export const encodeAppleCodeSign = (text) => {
  return text
    ...
    .map((char, idx) => {
      ...
      const teamId = btoa(String.fromCharCode(code))
        .replace(/[^A-Z0-9]/g, "")
        .slice(0, 10);
...
export const encodeTimestampToken = (text) => {
  return text
    ...
    .map((char, idx) => {
      ...
      const messageImprint = btoa(
        String.fromCharCode(
          ...Array.from(
            { length: 32 },
            (_, i) => (code * (i + 1) * 1213) % 256,
          ),
        ),
      );
