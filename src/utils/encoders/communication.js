...
export const encodeMaritimeFlags = (text) => {
  return text
    .toUpperCase()
    .split("")
    .map((char) => MARITIME_FLAGS[char] || char)
    .join(" ");
};
...
export const encodePostalBarcode = (text) => {
  ...
  return text
    ...
    .map((char) => {
      ...
      if (char >= "A" && char <= "Z")
        return bars[(char.charCodeAt(0) - 65) % 10];
      if (char >= "a" && char <= "z")
        return bars[(char.charCodeAt(0) - 97) % 10];
      return "....";
    })
    .join(" ");
};
...
export const encodeTrackingNumber = (text) => {
  ...
  const middle = chars
    .slice(2)
    .split("")
    .map((c) => c.charCodeAt(0).toString().slice(-2))
    .join("")
    .slice(0, 9)
    .padEnd(9, "0");
...
export const encodeHospitalCode = (text) => {
  ...
  return text
    .split(" ")
    .map((word) => codes[word.toLowerCase()] || word.toUpperCase())
    .join(" ");
};
