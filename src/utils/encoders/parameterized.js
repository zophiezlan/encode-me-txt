...
export const encodeLeetspeakParam = (text, intensity = 1) => {
  const map =
    intensity >= 3 ? LEET_EXTREME : intensity >= 2 ? LEET_MEDIUM : LEET_BASIC;
  return text
    .split("")
    ...
    .map((word) => {
      ...
      const randomEmojis = Array(numEmojis)
        ...
        .map(
          () =>
            EMOJIPASTA_EMOJIS[
              Math.floor(Math.random() * EMOJIPASTA_EMOJIS.length)
            ],
        )
        .join("");
...
export const encodeBinaryParam = (text, groupSize = 8) => {
  ...
  return (
    binary.match(new RegExp(`.{1,${groupSize}}`, "g"))?.join(" ") || binary
  );
};
...
export const decodeROT5 = (text, shift = 5) =>
  encodeROT5(text, 10 - (shift % 10));
...
export const encodeNihilist = (text, keyword = "ZEBRA") => {
  ...
  const key =
    keyword
      ...
      .replace(/J/g, "I") || "ZEBRA";
  const uniqueKey = [...new Set(key.split(""))].join("");
  const remaining = "ABCDEFGHIKLMNOPQRSTUVWXYZ"
    .split("")
    .filter((c) => !uniqueKey.includes(c))
    .join("");
 ...
 */
export const encodeBookCipher = (
  text,
  book = "The quick brown fox jumps over the lazy dog and runs away quickly",
) => {
 ...
 */
export const encodeDoubleTransposition = (
  text,
  key1 = "FIRST",
  key2 = "SECOND",
) => {
 ...
 * @returns {string} - Decoded text
 */
export const decodeDoubleTransposition = (
  text,
  key1 = "FIRST",
  key2 = "SECOND",
) => {
...
export const encodeNumericBase = (text, base = 10, separator = "-") => {
  ...
  return text
    .split("")
    .map((char) => char.charCodeAt(0).toString(clampedBase).toUpperCase())
    .join(separator);
};
...
export const decodeNumericBase = (text, base = 10, separator = "-") => {
  ...
  try {
    return text
      .split(separator)
      .map((num) => String.fromCharCode(parseInt(num, clampedBase)))
      .join("");
  } catch {
    return "[Decode failed]";
  }
 ...
 * @returns {string} - Transformed text
 */
export const encodeVowelConsonant = (
  text,
  target = "vowels",
  operation = "upper",
  replacement = "*",
...
export const encodeRLE = (text, format = "standard") => {
  ...
  switch (format) {
    case "compact":
      return runs
        .map((r) => (r.count > 1 ? `${r.count}${r.char}` : r.char))
        .join("");
    case "verbose":
...
export const decodeRLE = (text, format = "standard") => {
  try {
    switch (format) {
      case "compact": {
        return text.replace(/(\d+)(.)/g, (_, count, char) =>
...
export const encodeHuffmanStyle = (text) => {
  ...
  const dict = sorted
    .map(
      ([c]) => `${c.charCodeAt(0).toString(16).padStart(2, "0")}:${codes[c]}`,
    )
    .join(",");
...
export const encodeGeneticAlgorithm = (text, generations = 3) => {
  ...
  const chromosome = genes
    .map((g) => `[${g.allele}|f:${g.fitness}|g${g.generation}]`)
    .join("");
...
export const encodeFractalDimension = (text, depth = 2) => {
  ...
  const patterns = text
    .split("")
    .map((c) => fractalPattern(c.charCodeAt(0), clampedDepth));
    ...
    .map((char) => {
      ...
      switch (format) {
        case "timing": {
          ...
          const intervals = binary
            .split("")
            .map((b, i) => (b === "1" ? (i + 1) * 10 : 0))
            .filter((t) => t > 0);
  ...
  try {
    switch (style) {
      case "arrow": {
        ...
        return matches
          .map((m) => String.fromCharCode(parseInt(m.match(/(\d+)/)[1])))
          .join("");
      }
      case "haskell": {
        const matches = text.match(/=\s*(\d+)/g) || [];
      ...
      case "polar": {
        ...
        return matches
          .map((m) => {
            ...
            const x = Math.min(
              15,
              Math.max(0, Math.round(r * Math.cos(theta))),
            );
            const y = Math.min(
              15,
              Math.max(0, Math.round(r * Math.sin(theta))),
            );
