...
export const encodeVariationSelector = (text) => {
  ...
  return text
    ...
    .map((char) => {
      ...
      return (
        carrier +
        String.fromCharCode(VS_BASE + nibble1) +
        carrier +
        String.fromCharCode(VS_BASE + nibble2)
      );
    })
    .join("");
};
...
export const decodeNullByte = (text) => {
  try {
    ...
    return matches
      .map((m) => String.fromCharCode(parseInt(m.slice(3), 16)))
      .join("");
  } catch {
    return "[Decode failed]";
  }
...
export const decodeRegexPattern = (text) => {
  try {
    ...
    if (!matches) return "[Invalid format]";
    return matches
      .map((m) => String.fromCharCode(parseInt(m.slice(2), 16)))
      .join("");
  } catch {
    return "[Decode failed]";
  }
