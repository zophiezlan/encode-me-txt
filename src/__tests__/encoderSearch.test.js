...
describe('searchEncoders', () => {
  ...
  it('filters by multiple categories', () => {
    const results = searchEncoders(mockEncoders, { categories: ['cipher', 'fun'] });
    expect(results).toHaveLength(2);
  });

  it('filters by tags (AND logic)', () => {
    const results = searchEncoders(mockEncoders, { tags: ['classic', 'morse'] });
    expect(results).toHaveLength(1);
    expect(results[0].id).toBe('morse-pro');
  });

  it('filters by anyTags (OR logic)', () => {
    const results = searchEncoders(mockEncoders, { anyTags: ['steganography', 'horror'] });
    expect(results).toHaveLength(2);
  });
